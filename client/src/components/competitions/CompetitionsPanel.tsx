...
interface CompetitionsPanelProps {
  /** Скрыть внешний заголовок страницы (когда вложено в Календарный план). */
  embedded?: boolean;
}

const CompetitionsPanel: React.FC<CompetitionsPanelProps> = ({ embedded = false }) => {
    ...
    <LocalizationProvider dateAdapter={AdapterDateFns} adapterLocale={ru}>
      <Box>
        ...
        <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', mb: 3 }}>
          <Typography variant="h5">Соревнования</Typography>
          <Button
            variant="contained"
            startIcon={<Add />}
            onClick={() => {
